...
public class ShapeInfoController implements Initializable {
  ...
  public void updateInfo(Mshape mshape) {
    ...
    colorPicker.setValue(Color.rgb(mshape.getColor().getRed(), mshape.getColor().getGreen(),
            mshape.getColor().getBlue(), mshape.getColor().getAlpha() / 255));
  }

  private void addListeners() {
    xPosition.textProperty().addListener(
            (observable, oldValue, newValue) -> {
              if (!xPosition.getText().isEmpty()) {
                Command command = new Move(Integer.parseInt(oldValue),
                        Integer.parseInt(yPosition.getText()), Integer.parseInt(newValue),
    ...
    yPosition.textProperty().addListener(
            (observable, oldValue, newValue) -> {
              if (!yPosition.getText().isEmpty()) {
                Command command = new Move(Integer.parseInt(xPosition.getText()),
